...
class FootballBettingAid(object):
    ...
    def diagnose(self):
        ...
        with PdfPages(os.path.join(self.results_dir, 'diagnostics_{}_{}_{}_{}.pdf'.format(
            ...
            plt.grid(True)
            plt.hlines(xmax=max(df_coefs.index), xmin=min(df_coefs.index), y=0, linestyles='dashed')
            ...
            plt.grid(True)
            plt.hlines(xmax=max(df_globals.index), xmin=min(df_globals.index), y=0, linestyles='dashed')
            plt.title('Globals')
            plt.xticks(rotation=90)
            plt.tight_layout()
            pdf.savefig()
